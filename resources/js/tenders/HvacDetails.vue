...
<div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
 ...
 <strong>{{company_name}}</strong>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-success" @click="confirmOffer(Ifconfirm,com_id)">تأكيد</button>
      </div>
    </div>
  </div>
</div>
...
<span :v-model="vendor(comment)"></span>
    ...
    <div class=" " v-if="IsUser!=10 && tender.stage==0">


<form @submit.prevent="SubmitOffer" enctype="multipart/form-data" v-if="MyofferIsset==0" class="p-descrip-box mt-3">
  ...
  <tbody>
    <tr>
        ...
        <td class="text-center">
            <a :href="tender.hvac_files.threeDPath" target="_blank" class="btn btn-download">
            <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">معاينة </small>
            </a>
                       <a v-if="tender.hvac_files.ThreeDDWG" :href="tender.hvac_files.ThreeDDWG" class="btn btn-success">
                <i class="bx bxs-down-arrow-circle"></i> ملف الاوتوكاد 
            </a>
             <button v-else-if="IsUser==10"  @click="HandleFile('threed',tender.hvac_files.threeDName,tender.hvac_files.id)"
    ...
    <tr>
      <td>المخطط المعماري</td>
        <td class="text-center"><a :href="tender.hvac_files.ArchitectPath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">معاينة </small>
            </a>
                        <a v-if="tender.hvac_files.ArchitectDWG" :href="tender.hvac_files.ArchitectDWG" class="btn btn-success">
                <i class="bx bxs-down-arrow-circle"></i> ملف الاوتوكاد 
            </a>
                <button v-else-if="IsUser==10"  @click="HandleFile('Architect',tender.hvac_files.ArchitectName,tender.hvac_files.id)
    ...
    <tr>
      ...
      <td> الملف الانشائي</td>
        <td class="text-center"><a :href="tender.hvac_files.StructurePath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">معاينة </small>
            </a>
              <a v-if="tender.hvac_files.StructureDWG" :href="tender.hvac_files.StructureDWG" class="btn btn-success">
                <i class="bx bxs-down-arrow-circle"></i> ملف الاوتوكاد 
            </a>
               <button v-else-if="IsUser==10"  @click="HandleFile('Structure',tender.hvac_files.StructureName,tender.hvac_files.id)" class="btn btn-light"
      ...
      <td>جدول المواصفات</td>
        <td class="text-center"><a :href="tender.hvac_files.SpecificationsPath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">معاينة </small>
            </a>
              <a v-if="tender.hvac_files.SpecificationsEXCEL" :href="tender.hvac_files.SpecificationsEXCEL" class="btn btn-success">
                <i class="bx bxs-down-arrow-circle"></i> ملف الاكسل 
            </a>
                <button v-else-if="IsUser==10"  @click="HandleFile('spec',tender.hvac_files.SpecificationsName,tender.hvac_files.id)" class="btn btn-light"
     ...
     <tr>
      ...
      <td> الهوية</td>
        <td class="text-center"><a :href="tender.hvac_files.identityPath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">معاينة </small>
            </a>
             <button v-if="IsUser==10"  @click="HandleFile('ident',tender.hvac_files.identityName,tender.hvac_files.id)" class="btn btn-light"
      ...
      <td> خارطة الارض</td>
        <td class="text-center"><a :href="tender.hvac_files.mapPath" download="" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">معاينة </small>
            </a>
            <button v-if="IsUser==10"  @click="HandleFile('map',tender.hvac_files.mapName,tender.hvac_files.id)" class="btn btn-light"
...
successAlert(){
        ...
        //               }
                ...
                if(event == 'threeD'){
                    ...
                    axios.post('update/hvac/threedFile',data,config).then(res=>{
...
console.log(result1);
  ...
  files.splice(i,1,'');
    console.log(files);
               ...
               let data = new FormData();
                  ...
                  let UpdateNotes = document.querySelector("textarea[name=desc]").value
                   let Updatetitle = document.querySelector("input[name=title]").value;
                    ...
                    axios.post('UpdateTitleHvac',data).then(res=>{
        ...
        axios.post('confirm/hvac/offer',data).then(res=>{
            this.getDetails();
...
console.log(result1);
 ...
 if(this.approval==true){
    ...
    UpdatePost() {
      ...
      axios
        .post("/UpdateTenderTitle", formdata /*config*/)
        .then(res => {
    ...
    getDetails() {
      axios
        .get(`/showHvacTender/${this.$route.params.id}`)
        .then(res=>{
