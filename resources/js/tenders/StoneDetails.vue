...
<span v-if="resultadditionalSize!=null" ><span class="fa fa-file" style="color:#ebeef2"  ></span>  {{files2[i].name}}</span>
          ...
          <small class="text-muted">Pdf , dwg (autocad file) , jpg , png)</small>
             <div class="file-style ">
                 <label>
                 <span class="btn btn-download" @click="$refs.add" v-if="additional==null"><span class=" fa fa-plus" ></span> </span>
...
<div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
      <strong>{{company_name}}</strong> هل انت متأكدر تريد أختيار عرض 
      </div>
      <div class="modal-footer">
        <button type="button" class="theme-btn primary" data-dismiss="modal">إغلاق</button>
        <button type="button" class="theme-btn" @click="confirmOffer(Ifconfirm,com_id)">اختيار</button>
      </div>
    </div>
  </div>
</div>
          ...
          <li class="list-group-item container">
           <status :status="tender.status"></status>
            ...
            <div class="d-block mt-3">
                 ...
                 <span > <i class="fa fa-circle text-success  mt-4" style="font-size:14px"></i></span>
 ...
 <div class="h5 text-right"> ({{tender.comments.length}}) عدد العروض </div>
                ...
                <span v-for="comment,ie in tender.comments" :key="ie" v-bind="comment" class=" mt-3">
                    ...
                    <div class=" mt-3 ">
                        ...
                        <div class="text-right h5">المقترح</div>
                        <p class="text-right">{{comment.textOffer}}</p>
                        <div class="text-right h5">البنود الغير مشمولة</div>
...
<!--
                     ...
                     <div class="h5 text-right">ملف المقترح</div>
...
<button class="btn btn-success  confirm" @click.prevent="confirm(comment.id,comment.user.name,comment.user.id)"
...
<table class="table table-striped" v-if="userid==confirm.com_id && tender.stage==1  && confirm.offer_id ==comment.id">
  <thead>
    <tr>
  ...
  <tbody>
     <tr>
      ...
      <td>{{tender.user.mobile}}</td>
             ...
             v-if="userid==confirm.com_id && tender.stage==1  && confirm.offer_id ==comment.id" disabled >
...
<span :v-model="vendor(comment)"></span>
    ...
    <div  v-if="IsUser!=10 && tender.stage==0">


<form @submit.prevent="SubmitOffer" enctype="multipart/form-data" class="p-descrip-box mt-3" v-if="MyofferIsset==0">
...
<input type="range" id="points" name="points" min="1000" max="5000" v-model="rangeval"> -->
         ...
         <button type="submit" class="ml-3  text-light btn btn-download mt-3"
  ...
  <tbody>
    <tr>
         <td>  3D </td>
        <td class="text-center"><a :href="tender.stone_files.threeDPath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">مشاهدة </small>
            </a>
                   <a v-if="tender.stone_files.ThreeDDWG" :href="tender.stone_files.ThreeDDWG" class="btn btn-success">
                <i class="bx bxs-down-arrow-circle"></i> ملف الاوتوكاد 
            </a>
              
             <button v-else-if="IsUser==10"  @click="HandleFile('threed',tender.stone_files.threeDName,tender.stone_files.id)"
    ...
    <tr>
      <td>المخطط المعماري</td>
        <td class="text-center"><a :href="tender.stone_files.ArchitectPath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">مشاهدة </small>
            </a>
               <a v-if="tender.stone_files.ArchitectDWG" :href="tender.stone_files.ArchitectDWG" class="btn btn-success">
                <i class="bx bxs-down-arrow-circle"></i> ملف الاوتوكاد 
            </a>
                 <button v-else-if="IsUser==10"  @click="HandleFile('Architect',tender.stone_files.ArchitectName,tender.stone_files.id)
    ...
    <tr>
      ...
      <td> الملف الانشائي</td>
        <td class="text-center"><a :href="tender.stone_files.StructurePath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">مشاهدة </small>
            </a>
               <a v-if="tender.stone_files.StructureDWG" :href="tender.stone_files.StructureDWG" class="btn btn-success">
                <i class="bx bxs-down-arrow-circle"></i> ملف الاوتوكاد 
            </a>
              <button v-else-if="IsUser==10"  @click="HandleFile('Structure',tender.stone_files.StructureName,tender.stone_files.id)" class="btn btn-light"
      ...
      <td>جدول المواصفات</td>
        <td class="text-center"><a :href="tender.stone_files.SpecificationsPath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">مشاهدة </small>
            </a>
            <a v-if="tender.stone_files.SpecificationsEXCEL" :href="tender.stone_files.SpecificationsEXCEL" class="btn btn-success">
                <i class="bx bxs-down-arrow-circle"></i> ملف الأكسل 
            </a>
             <button v-else-if="IsUser==10"  @click="HandleFile('spec',tender.stone_files.SpecificationsName,tender.stone_files.id)" class="btn btn-light"
     ...
     <tr>
      ...
      <td> الهوية</td>
        <td class="text-center"><a :href="tender.stone_files.identityPath" download="" class="btn btn-download">
              <i class=" fas fa-cloud-download-alt" style="font-size:10px"></i>
            <small class="">تحميل </small>
            </a>
             <button v-if="IsUser==10"  @click="HandleFile('ident',tender.stone_files.identityName,tender.stone_files.id)" class="btn btn-light"
      ...
      <td> خارطة الارض</td>
        <td class="text-center"><a :href="tender.stone_files.mapPath" target="_blank" class="btn btn-download">
           <i class=" fas fa-eye" style="font-size:10px"></i>
            <small class="">مشاهدة </small>
            </a>
              <button v-if="IsUser==10"  @click="HandleFile('map',tender.stone_files.mapName,tender.stone_files.id)" class="btn btn-light"
...
export default {
  components: { stages, TitleTender, AllUae, Status },
  data() {
    return {
      ...
      methods:{
        ...
        //               }
                ...
                if(event == 'threeD'){
                    ...
                    axios.post('update/stone/threedFile',data,config).then(res=>{
               ...
               let data = new FormData();
                  ...
                  let UpdateNotes = document.querySelector("textarea[name=desc]").value
                   let Updatetitle = document.querySelector("input[name=title]").value;
                    ...
                    axios.post('UpdateTitleStone',data).then(res=>{
...
console.log(result1);
  ...
  files.splice(i,1,'');
    ...
    this.progress5=0;
        ...
        axios.post('confirm/stone/offer',data).then(res=>{
            this.getDetails();
...
console.log(result1);
 ...
 if(this.approval==true){
        ...
        let insert = ''
            ...
            axios.post('insertStoneOffer',data).then(res=>{
    ...
    getDetails() {
      axios
        .get(`/showStoneTender/${this.$route.params.id}`)
        .then( response =>{
